...
class TestDataSufficiencyGuards:
    """
    Tests for data sufficiency guards.
    
    These guards ensure deterministic behavior with insufficient historical data.
    
    GUARD CONDITIONS:
    1. Signal detection: If 1m candles < 120, return None (no logging, no partial signal)
    2. Quality calculation: If 5m candles < 60 OR idx5 < 55, force zero for ADX/slope/trend
    """
    ...
    def _create_1m_candles(self, n: int, start_ts: int = 1000) -> list[Candle]:
        """Create n 1m candles with touch+confirm pattern at the end."""
        candles = []
        base_price = 100
        for i in range(n):
            price = base_price + i * 0.01
            # For the last few candles, create a touch+confirm pattern
            if i >= n - 3 and n >= 22:
                # Setup pattern: low touches EMA, close above EMA, next bar confirms
                candles.append(Candle(
                    t=start_ts + i * 60,
                    o=price,
                    h=price + 1,
                    l=price - 2 if i == n - 2 else price - 0.5,  # Low touches EMA
                    c=price + 0.5,  # Close above EMA
                    v=1000,
                    close_time=start_ts + (i + 1) * 60 - 1,
                ))
            else:
    ...
    def _create_5m_candles(self, n: int, start_ts: int = 1000) -> list[Candle]:
        """Create n 5m candles with uptrending pattern."""
        candles = []
        base_price = 1000
        for i in range(n):
            price = base_price + i * 0.5
    ...
    # ===== 1m Candles Signal Detection Guard Tests =====
    
    def test_no_signal_when_1m_candles_less_than_120(self, ta_engine):
        """No signal when 1m candles < 120 (hard gate)."""
        # Test with 119 candles (just below threshold)
        candles_1m = self._create_1m_candles(119)
        
        result = ta_engine.detect_signal(candles_1m, start_ts=1000)
        
        # Must return None with insufficient data
        assert result is None
    
    def test_no_signal_when_1m_candles_far_below_threshold(self, ta_engine):
        """No signal when 1m candles is far below 120."""
        # Test with 50 candles
        candles_1m = self._create_1m_candles(50)
        
        result = ta_engine.detect_signal(candles_1m, start_ts=1000)
        
        assert result is None
    
    def test_signal_possible_when_1m_candles_at_120(self, ta_engine):
        """Signal detection proceeds when 1m candles >= 120."""
        # Test with exactly 120 candles
        candles_1m = self._create_1m_candles(120)
        
        # Should NOT return None due to insufficient data
        # (may still return None if no signal pattern found)
        result = ta_engine.detect_signal(candles_1m, start_ts=1000)
        
        # The key is that detect_signal runs the full logic
        # It may return None for "no pattern" but not for "insufficient data"
        # We can verify by checking the guard constant
        assert ta_engine.MIN_1M_CANDLES_FOR_SIGNAL == 120
    
    def test_signal_possible_when_1m_candles_above_120(self, ta_engine):
        """Signal detection proceeds when 1m candles > 120."""
        # Test with 150 candles (above threshold)
        candles_1m = self._create_1m_candles(150)
        
        # Should NOT return None due to insufficient data
        assert ta_engine.MIN_1M_CANDLES_FOR_SIGNAL == 120
        assert len(candles_1m) >= ta_engine.MIN_1M_CANDLES_FOR_SIGNAL
    
    # ===== 5m Candles Quality Calculation Guard Tests =====
    
    def test_quality_forced_zero_when_5m_candles_less_than_60(self, ta_engine):
        """When 5m candles < 60, force q_adx=0, q_slope=0, trend_mult=1.0."""
        signal = SignalResult(
            direction=Direction.UP,
            signal_ts=5000,
            signal_price=1010,
        ...
        # Create only 59 candles (just below threshold)
        candles_5m = self._create_5m_candles(59)
        
        q = ta_engine.calculate_quality(signal, candles_5m)
        
        # ADX, slope, and trend must be forced to zero/neutral
        assert q.q_adx == 0.0
        assert q.q_slope == 0.0
        assert q.trend_mult == 1.0
        assert q.insufficient_5m_data == True
    
    def test_quality_forced_zero_when_idx5_less_than_55(self, ta_engine):
        """When idx5 < 55, force q_adx=0, q_slope=0, trend_mult=1.0."""
        signal = SignalResult(
            direction=Direction.UP,
            signal_ts=5000,  # Early signal_ts to get low idx5
            signal_price=1010,
            anchor_bar_ts=1000,
            anchor_price=1000,
            signal_bar_index=20,
        ...
        # Create 100 candles but with signal_ts that gives idx5 < 55
        # Signal_ts=5000, candles start at ts=1000 with 300s intervals
        # idx5 will be around (5000-1000)/300 = 13, which is < 55
        candles_5m = self._create_5m_candles(100)
    ...
    def test_quality_computed_when_guards_satisfied(self, ta_engine):
        """Normal quality computation when all guards are satisfied."""
        # Signal_ts needs to give idx5 >= 55
        # With start_ts=1000 and 300s intervals, we need signal_ts >= 1000 + 55*300 = 17500
        signal = SignalResult(
            direction=Direction.UP,
            signal_ts=20000,  # This gives idx5 around 63
            signal_price=1010,
            anchor_bar_ts=1000,
            anchor_price=1000,
            signal_bar_index=20,
        ...
        # Create 100 candles (>= 60)
        candles_5m = self._create_5m_candles(100)
        
        q = ta_engine.calculate_quality(signal, candles_5m)
        
        # With sufficient data, quality components should be computed normally
        # (not forced to zero)
        assert q.insufficient_5m_data == False
        # ADX and slope might still be 0 if data is flat, but trend_mult should not be neutral
        # unless close equals EMA20 exactly
        assert q.trend_mult in [1.10, 0.70, 1.00]
    
    def test_quality_constants_match_spec(self, ta_engine):
        """Verify data sufficiency guard constants match specification."""
        assert ta_engine.MIN_1M_CANDLES_FOR_SIGNAL == 120
        assert ta_engine.MIN_5M_CANDLES_FOR_QUALITY == 60
        assert ta_engine.MIN_IDX5_FOR_QUALITY == 55
